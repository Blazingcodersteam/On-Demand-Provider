...
import '../../Controller/availabily_range_controller.dart';
import '../../Controller/logincontroller.dart';
import '../commonpage/export.dart';

class Availabilityrange extends StatefulWidget {
  const Availabilityrange({super.key});

  @override
  State<Availabilityrange> createState() => _AvailabilityrangeState();
}

class _AvailabilityrangeState extends State<Availabilityrange> {
  ...
  Availabilityrangecontroller availabilityrangecontroller = Get.put(Availabilityrangecontroller());
  @override
  void initState() {
  ...
  searchDeliveryAddress() {
    try{
      GeocodingPlatform.instance!.locationFromAddress(searchAddress.text.isNotEmpty?"${searchAddress.text},india":"").then((result) {
        if (result.isEmpty) {
          Get.snackbar(
            "Availability Status",
          ...
          _getAddressFromLatLng(LatLng(result.first.latitude, result.first.longitude)).whenComplete(() => searchAddress.clear());

          Get.snackbar(
            "Availability Status",
            "Tap your accurate place to select the right address for your delivery",
            //icon: Icon(Icons.person, color: Colors.white),
            snackPosition: SnackPosition.BOTTOM,
        ...
        // _getAddressFromLatLng(LatLng(result[0].latitude, result[0].longitude));
  ...
  Widget build(BuildContext context) {
    if(currentLocationStatus && availabilityrangecontroller.availabilityRangeStatus.isFalse) {
      return Scaffold(
                    ...
                    Container(
                      ...
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          hintText: 'Enter Area',
                          hintStyle:ThemeText.bodydata.merge(TextStyle(fontSize: 15)),
                          ...
                          //   borderSide: const BorderSide(width: 1, color: Color(0xff293E6F)),
                          ...
                          // floatingLabelStyle: const TextStyle(color: Color(0xff293E6F)),
                        ),
                      ),
                    ),
                  ],
                  ...
                  Container(
                    ...
                    child: SizedBox(
                      ...
                      child: ElevatedButton(
                        ...
                        onPressed: () {
                          availabilityrangecontroller.availabilityscreen({
                               "user_id": Getstore.box.read("userid").toString(),
                               "range": (city==null)?"":city,
                          }, "service_range_save");
